...
shinyServer(function(input, output, session) {
  consumer_api_key <-
    "XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX" # Enter your consumer key
  consumer_api_secret <-
    "XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX" # Enter your consumer secret
  access_token <-
    "XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX" # Enter your access token
  access_token_secret <-
    "XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX" # Enter your access secret
  twitteR::setup_twitter_oauth(consumer_api_key,
                               consumer_api_secret,
                               access_token,
  ...
  TweetFrame <- function(twtList)
  {
    ...
    df$text <-
      sapply(df$text, function(row)
        iconv(row, "latin1", "ASCII", sub = "")) #If you wish to print emoticons just comment this line
  ...
  {
    corpus <-
      VCorpus(VectorSource(text)) #Fixed Corpus Transformation issue
    #clean text
  ...
  output$histPos <-
    renderPlot({
      hist(
        table_final()$Positive,
        col = rainbow(10),
        main = "Histogram of Positive Sentiment",
        xlab = "Positive Score"
      )
    })
  output$histNeg <-
    renderPlot({
      hist(
        table_final()$Negative,
        col = rainbow(10),
        main = "Histogram of Negative Sentiment",
        xlab = "Negative Score"
      )
    })
  output$histScore <-
    renderPlot({
      hist(
        table_final()$Score,
        col = rainbow(10),
        main = "Histogram of Score Sentiment",
        xlab = "Overall Score"
      )
    })
  ...
  slices <-
    reactive ({
      slices <-
        c(sum(table_final()$Positive), sum(table_final()$Negative))
    })
  ...
  toptrends <- function(place)
  {
    ...
    dat3 <-
      grep("dat2", iconv(dat2, "latin1", "ASCII", sub = "dat2"))
    dat4 <- dat2[-dat3]
    return (dat4)
  }
  ...
  trend_table <-
    reactive({
      trend_table <- toptrends(input$trendingTable)
    })
  ...
  toptweeters <- function(tweetlist)
  {
    ...
    d <-
      d[order(d$Freq, decreasing = T), ] #descending order of top charts according to frequency of tweets
  ...
  d <- reactive({
    d <- toptweeters(twtList())
  })
  output$tweetersplot <-
    renderPlot (
      barplot(
        head(d()$Tweets, 20),
        names = head(d()$User, 20),
        horiz = F,
        las = 2,
        main = "Top 20 users associated with the Hashtag",
        col = 1
      )
    )
  ...
  tw1 <- reactive({
    tw1 = userTimeline(input$user, n = 3200)
  })
  tw <- reactive({
    tw = twListToDF(tw1())
  })
  ...
  output$tophashtagsplot <- renderPlot ({
    p()
  })
}) #shiny server
